...
numCores = detectCores() # to detect the number of cores - 4 on pc, 24 on apollo
registerDoParallel(numCores) 
# to register the parallel backend with the foreach package
...
# parallellising the process of simulating 100 original datasets and applying 
# SigClust to each one of them
pvalue1 = foreach (b=1:100, .packages=c('MASS', 'sigclust')) %dopar%{
  set.seed(1+b)
  dat = mvrnorm(n=n, mu=mu, Sigma=D) # n=40 p-dimensional observations
  nsim = 1000 
  nrep = 1 #single random start in k-means
  icovest = 3 #hard-thresholding
...
#Plots
rm(list=ls())
setwd('C:/Users/Steve/OneDrive - Imperial College London/MSc/Summer Project/R Code/Huang d_300/single_hard')
load("pvalues.RData")
...
D[1, 1] = v[1] # v = 1
Delta = sum(pvalue1[[1]]@vsimeigval)-sum(diag(D))
delta1 = pvalue1[[1]]@vsimeigval[1]-D[1, 1]
Delta * (D[1, 1]/sum(diag(D))) > delta1
TCI = 1 - ((2/pi)*(D[1, 1]/sum(diag(D))))
TCI_est = 1 - ((2/pi)*(pvalue1[[1]]@vsimeigval[1]/sum(pvalue1[[1]]@vsimeigval)))

D[1, 1] = v[4] # v = 200
Delta = sum(pvalue4[[5]]@vsimeigval)-sum(diag(D))
delta1 = pvalue4[[5]]@vsimeigval[1]-D[1, 1]
Delta * (D[1, 1]/sum(diag(D))) > delta1
TCI = 1 - ((2/pi)*(D[1, 1]/sum(diag(D))))
TCI_est = 1 - ((2/pi)*(pvalue1[[1]]@vsimeigval[1]/sum(pvalue1[[1]]@vsimeigval)))
