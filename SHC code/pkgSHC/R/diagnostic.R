...
diagnostic.shc <- function(obj, K = 1, fname = NULL, ci_idx = 1,
    ...
    if (is.null(fname)) {
        ...
        for (k in K) {
            plot(0, 0, col="white")
            text(0, 0, paste("K =", k))
            .diagnostic_k(obj, nrow(obj$p_norm)+1, k, ci_idx, pty)
        }
        dev.off()
    }
}
...
.diagnostic_k <- function(obj, ntot, k, ci_idx, pty) {

    ## identify subtree of x
    idx_sub <- unlist(obj$idx_hc[ntot-k, ])
    ...
    keigval_dat <- obj$eigval_dat[ntot-k, ]
    backvar_k <- obj$backvar[ntot-k]
    keigval_sim <- obj$eigval_sim[ntot-k, ]
    kci_sim <- obj$ci_sim[ntot-k, , ci_idx]
    kci_dat <- obj$ci_dat[ntot-k, ci_idx]
    kp_emp <- obj$p_emp[ntot-k, ci_idx]
    kp_norm <- obj$p_norm[ntot-k, ci_idx]
