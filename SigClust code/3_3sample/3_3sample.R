...
### CASE OF MIXTURE OF TWO GAUSSIAN DISTRIBUTIONS WITH SIGNAL IN ALL DIRECTIONS
###-------------------------------------------------------------------------- 
n = 40
d = 300
mu = rep(0, d)
...
pvalue1 = foreach (b=1:100, .packages=c('MASS', 'sigclust')) %dopar%{
  ...
  dat[(n/2+1):(n), 1:d] = dat[(n/2+1):(n), 1:d] + a[1]
  nsim = 1000
  nrep = 1
  icovest = 2
...
pvalue2 = foreach (b=1:100, .packages=c('MASS', 'sigclust')) %dopar%{
  set.seed(1+b)
  dat = mvrnorm(n=n, mu=mu, Sigma=D) # n=40 d-dimensional observations
  dat[(n/2+1):(n), 1:d] = dat[(n/2+1):(n), 1:d] + a[2]
  nsim = 1000
  nrep = 1
  icovest = 2
  pvalue = sigclust(dat, nsim=nsim, nrep=nrep, labflag=0, icovest=icovest)
...
pvalue3 = foreach (b=1:100, .packages=c('MASS', 'sigclust')) %dopar%{
  set.seed(1+b)
  dat = mvrnorm(n=n, mu=mu, Sigma=D) # n=40 d-dimensional observations
  dat[(n/2+1):(n), 1:d] = dat[(n/2+1):(n), 1:d] + a[3]
  nsim = 1000
  nrep = 1
  icovest = 2
  pvalue = sigclust(dat, nsim=nsim, nrep=nrep, labflag=0, icovest=icovest)
...
pvalue4 = foreach (b=1:100, .packages=c('MASS', 'sigclust')) %dopar%{
  set.seed(1+b)
  dat = mvrnorm(n=n, mu=mu, Sigma=D) # n=40 d-dimensional observations
  dat[(n/2+1):(n), 1:d] = dat[(n/2+1):(n), 1:d] + a[4]
  nsim = 1000
  nrep = 1
  icovest = 2
  pvalue = sigclust(dat, nsim=nsim, nrep=nrep, labflag=0, icovest=icovest)
...
#Plots
rm(list=ls())
setwd('C:/Users/Steve/OneDrive - Imperial College London/MSc/Summer Project/R Code/Huang d_300/3_3sample')
load("pvalues.RData")
...
p = ggplot() + stat_ecdf(dat=pvals, aes(x=pvalue, linetype=a, colour=a)) + 
  ...
  ggtitle("Mixture of Gaussians - Signal in All Directions (sample)") +
